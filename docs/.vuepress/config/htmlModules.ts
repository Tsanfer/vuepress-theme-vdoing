 ...
 *   pageTshowMode: string, 页面顶部-显示方式：未配置默认所有页面；'article' => 仅文章页1. ； 'custom' => 仅自定义页1. 
 *   pageBshowMode: string, 页面底部-显示方式：未配置默认所有页面；'article' => 仅文章页1. ； 'custom' => 仅自定义页1. 
 *
 *   windowLB: htmlString, 全局窗口左下角2. 
 *   windowRB: htmlString, 全局窗口右下角2. 
 * }
 *
 * 1. 注：在.md文件front matter配置`article: false`的页面是自定义页，未配置的默认是文章页（首页除外）。
 * 2. 注：windowLB 和 windowRB：1.展示区块最大宽高200px*400px。2.请给自定义元素定一个不超过200px*400px的宽高。3.在屏幕宽度小于960px时无论如何都不会显示。
 */
